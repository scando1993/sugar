...
| `CLAUDE.md` | each phase workspace | Phase 3b | Ralph agent instructions for that phase's subagent |
...
/tmp/myapp-phases/phase-a-types/
  CLAUDE.md        ← Ralph agent instructions (generated by Phase 3b)
  prd.json         ← User stories for this phase (generated by Phase 3b)
  progress.txt     ← Progress log + codebase patterns (initialized Phase 2)
  (repo files via git worktree)
```

**Claude Code**: launches parallel Agent calls — one per independent phase:
```
Agent("cd /tmp/myapp-phases/phase-a-types && follow CLAUDE.md")
Agent("cd /tmp/myapp-phases/phase-b-api && follow CLAUDE.md")
```

**Copilot**: launches parallel terminal processes:
```bash
(cd /tmp/myapp-phases/phase-a-types && claude --print < CLAUDE.md) &
(cd /tmp/myapp-phases/phase-b-api && claude --print < CLAUDE.md) &
wait
```

Each subagent is autonomous — it reads its `CLAUDE.md`, follows the Ralph loop, commits per story, and pushes when done.
